...
double findEucDistances(int centroidIndex, NumericVector path,
                        NumericVector dims);
...
//'
//' @return A list consisting of current features calculated in FeatureSet as
// well as measures of compactness, loop count, and loop dimensions
//'
// [[Rcpp::export]]
List addToFeatures(List FeatureSet, List LetterList, IntegerVector vectorDims)
{
    ...
    for (int i = 0; i < FeatureSet.size(); i++)
    {
        ...
        NumericVector num = Rcpp::as<NumericVector>(
            Rcpp::as<Rcpp::List>(FeatureSet[i])["lHalf"]);
        NumericVector num1 = Rcpp::as<NumericVector>(
            Rcpp::as<Rcpp::List>(FeatureSet[i])["rHalf"]);
        NumericVector dims = Rcpp::as<NumericVector>(vectorDims);
        NumericVector path = Rcpp::as<NumericVector>(
            Rcpp::as<Rcpp::List>(LetterList[i])["path"]);
        int centroidIndex = Rcpp::as<int>(
            Rcpp::as<Rcpp::List>(FeatureSet[i])["centroid_index"]);

        // FIND SUM OF EUC DISTANCES OF THIS SPECIFIC INDEX
        double averageDistFromCentroid =
            findEucDistances(centroidIndex, path, dims);
        compactness.push_back(averageDistFromCentroid);

        // FIND LOOPS
        List allPaths = Rcpp::as<Rcpp::List>(
            Rcpp::as<Rcpp::List>(LetterList[i])["allPaths"]);
        ...
        List loopInfo = Rcpp::List::create(
            Rcpp::Named("loopRatio") = loopRatio,
            Rcpp::Named("loopPlottingInfo") = loopDimensions,
            Rcpp::Named("loopCount") = loopCounter);
        loopInfoList.push_back(loopInfo);
    }

    // Currently returns compactness of a letter, loopCount of a letter, and
    // dimensions of the loop.
    return Rcpp::List::create(Rcpp::Named("compactness") = compactness,
                              Rcpp::Named("loopInfo") = loopInfoList);
}

// Given a list of loop dimensions, return the list of ratios of height to
// width of each loop
List findLoopRatios(List loopDimensions, NumericVector dims)
{
    ...
    for (int i = 0; i < loopDimensions.size(); i++)
    {
        ...
        double longestLineDist =
            sqrt(pow(longestLineP2[0] - longestLineP1[0], 2) +
                 pow(longestLineP2[1] - longestLineP1[1], 2.0));

        List shortestLine = loopToProcess[2];
        NumericVector shortestLineP1 = shortestLine[0];
        NumericVector shortestLineP2 = shortestLine[1];
        double shortestLineDist =
            sqrt(pow(shortestLineP2[0] - shortestLineP1[0], 2) +
                 pow(shortestLineP2[1] - shortestLineP1[1], 2.0));
...
// Given a list of loops, return a list of vectors that has the points of the
// longest line through the centroid and the line perpendicular to it
List findLoopDims(List knownLoops, NumericVector dims)
{
    ...
    for (int i = 0; i < knownLoops.size(); i++)
    {
        NumericVector pathConsidering =
            Rcpp::as<NumericVector>(knownLoops[i]);
        ...
        {
            NumericVector pointXY =
                convertIndextoXY(pathConsidering[j], dims, 1);
            if (pointXY[0] == centroid[0] || pointXY[1] == centroid[1])
                continue;

            // Find slope between point and centroid
            double slopeFromXYToCentroid =
                ((centroid[1] - pointXY[1]) / (centroid[0] - pointXY[0]));
            double yIntercept =
                pointXY[1] - (slopeFromXYToCentroid * pointXY[0]);

            // Go through every other point
            for (int k = 0; k < pathConsidering.size(); k++)
            {
                NumericVector checkPointXY =
                    convertIndextoXY(pathConsidering[k], dims, 1);

                // Does it fall on the line?
                double distanceFromLine = fabs(
                    checkPointXY[1] -
                    ((slopeFromXYToCentroid * checkPointXY[0]) + yIntercept));

                if (distanceFromLine < 2)
                {  // only check distance if it is more on the line that a
                   // previous one.
                    // get distance between two points, if it is bigger than
                    // max then update max
                    int distance =
                        findDistanceBetweenTwoPoints(pointXY, checkPointXY);
                    if (distance > longestLineDistance)
                    {
                        longestLineDistance = distance;
                        longestLine =
                            Rcpp::List::create(pointXY, checkPointXY);
                    }
                }
            }
        }
        ...
        // once you found the longest find its perpendicular line and use that
        // as the shortest line
        double longestLineSlope = ((lengthPoint2[1] - lengthPoint1[1]) /
                                   (lengthPoint2[0] - lengthPoint1[0]));
        double shortestLineSlope = -1.0 / longestLineSlope;
        double yIntercept = centroid[1] - (shortestLineSlope * centroid[0]);

        // These variables will keep track of the closest matches as we go
        // around USE AFTER GET BASE WORKING --- THEN JUST FIND CLOSEST POINTS
        // ON BOTH SIDES
        ...
        // go through every point to find two points that fall on perp through
        // centroid
        for (int j = 0; j < pathConsidering.size() / 2; j++)
        {
            ...
            double distanceFromPerpLine =
                fabs(checkPointXY[1] -
                     ((shortestLineSlope * checkPointXY[0]) + yIntercept));
        ...
        for (int k = pathConsidering.size() / 2; k < pathConsidering.size();
             k++)
        {
            NumericVector checkPointXY =
                convertIndextoXY(pathConsidering[k], dims, 1);
        ...
        shortestLine =
            Rcpp::List::create(closestPerpPoint1, closestPerpPoint2);
        ...
        // Before we return, need to convert all column values back to
        // inverted
        ...
        longestLine = Rcpp::List::create(Rcpp::Named("P1") = invertedLP1,
                                         Rcpp::Named("P2") = invertedLP2);
        shortestLine = Rcpp::List::create(Rcpp::Named("P1") = invertedSP1,
                                          Rcpp::Named("P2") = invertedSP2);

        List loopInfoToAdd =
            Rcpp::List::create(Rcpp::Named("centroid") = centroid,
                               Rcpp::Named("longestLine") = longestLine,
                               Rcpp::Named("shortestLine") = shortestLine);
...
List findLoops(List allpaths, NumericVector dims)
{
    ...
    for (int i = 0; i < allpaths.size(); i++)
    {
        ...
        if (pathConsidering.size() < 10)
            continue;  // arbitarty value of 10 chosen, worked for every
                       // letter so far

        NumericVector startingPoint =
            convertIndextoXY(pathConsidering[0], dims);
        NumericVector endingPoint = convertIndextoXY(
            pathConsidering[pathConsidering.size() - 1], dims);
        double distanceBetweenPoints =
            findDistanceBetweenTwoPoints(startingPoint, endingPoint);
                        ...
                        NumericVector dims)
{
    ...
    double averageDistFromCentroid =
        (1.0 / ((double)path.size() - 1.0)) * sumOfDistances;
...
double findDistanceBetweenTwoPoints(NumericVector p1, NumericVector p2)
{
    ...
    int y_b = p1[1];
    int y_a = p2[1];
    double distance = sqrt(double((x_b - x_a) * (x_b - x_a)) +
                           double((y_b - y_a) * (y_b - y_a)));
    return distance;
}
...
NumericVector convertIndextoXY(int index, NumericVector dims, int flipY)
{
    ...
    if (flipY == 1) { colIndex = dims[0] - colIndex; }
    NumericVector XY = NumericVector::create(rowIndex, colIndex);
    return XY;
}

int indexToRow(int index, int rows) { return ((index - 1) / rows) + 1; }

int indexToCol(int index, int rows) { return ((index - 1) % rows) + 1; }
