...
MakeLetterListLetterSpecific <- function(letterList, dims) {
  ...
  for (i in 1:length(letterList))
  {
    ...
    # side of the letter
    letterList[[i]]$allPaths <- lapply(
      letterList[[i]]$allPaths,
      function(x) {
    ...
    # handwriting sample
    nameVect_i <- i_hs_to_i_letter(
      index_nums = as.numeric(colnames(letterList[[i]]$adjMatrix)),
      hs_num_rows = dims[1],
      letter_num_rows = letter_num_rows,
      letter_topmost_row = letter_topmost_row,
      letter_leftmost_col = letter_leftmost_col
    ...
    centroid_rc <- rc_hs_to_rc_letter(
      row_nums = letterList[[i]]$characterFeatures$centroid_y,
    ...
    # top and left of the letter.
    lHalf_rc <- i_hs_to_rc_letter(
      index_nums = letterList[[i]]$characterFeatures$lHalf,
      hs_num_rows = dims[1],
      letter_topmost_row = letter_topmost_row,
      letter_leftmost_col = letter_leftmost_col
    )
    ...
    # top and left of the letter.
    rHalf_rc <- i_hs_to_rc_letter(
      index_nums = letterList[[i]]$characterFeatures$rHalf,
      hs_num_rows = dims[1],
      letter_topmost_row = letter_topmost_row,
      letter_leftmost_col = letter_leftmost_col
    )
    ...
    # top and left of the letter
    letterList[[i]]$characterFeatures$lHalf <- i_hs_to_i_letter(
      index_nums = letterList[[i]]$characterFeatures$lHalf,
      hs_num_rows = dims[1],
      letter_num_rows = letter_num_rows,
      letter_topmost_row = letter_topmost_row,
    ...
    letterList[[i]]$characterFeatures$rHalf <- i_hs_to_i_letter(
      index_nums = letterList[[i]]$characterFeatures$rHalf,
      hs_num_rows = dims[1],
      letter_num_rows = letter_num_rows,
      letter_topmost_row = letter_topmost_row,
    ...
    bottom_right_rc <- rc_hs_to_rc_letter(
      row_nums = letterList[[i]]$characterFeatures$bottom_row,
    ...
    # relative to the top and left of the letter
    letterList[[i]]$path <- i_hs_to_i_letter(
      index_nums = letterList[[i]]$path,
      hs_num_rows = dims[1],
      letter_num_rows = letter_num_rows,
      letter_topmost_row = letter_topmost_row,
      letter_leftmost_col = letter_leftmost_col
    ...
    # relative to the top and left of the letter
    letterList[[i]]$nodes <- i_hs_to_i_letter(
      index_nums = letterList[[i]]$nodes,
      hs_num_rows = dims[1],
      letter_num_rows = letter_num_rows,
      letter_topmost_row = letter_topmost_row,
      letter_leftmost_col = letter_leftmost_col
...
MakeCenterStarts <- function(procList, K, numPathCuts, imagesList) {
  ...
  procList <- lapply(procList, function(x) {
...
centeredImageOnCentroid <- function(letter) {
  ...
  # of the letter image
  letter$pathEndsrc <- lapply(letter$allPaths, function(z) {
    cbind(((z[c(1, length(z))] - 1) %/% dim(letter$image)[1]) + 1, dim(letter$image)[1] - ((z[c(1, length(z))] - 1) %% dim(letter$image)[1]))
  })
  ...
  letter$pathEndsrc <- lapply(letter$pathEndsrc, function(z) {
    z - matrix(rep(letter$centroid, each = 2), ncol = 2)
  })
  return(letter)
}
