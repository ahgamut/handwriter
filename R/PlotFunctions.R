...
plotImage <- function(x) {
  ...
  p <- ggplot(xm, aes(Var2, rev(Var1))) +
    geom_raster(aes(fill = as.factor(value)), na.rm = TRUE) +
...
plotImageThinned <- function(img, thinned) {
  ...
  l.m$value[thinned] <- 2
  p <- ggplot(l.m, aes(Var2, rev(Var1))) +
    geom_raster(aes(fill = as.factor(value), alpha = as.factor(value)), na.rm = TRUE) +
    scale_alpha_manual(values = c(.1, NA, 1), guide = "none") +
    scale_fill_manual(values = c("black", "white", "black"), guide = "none") +
    coord_fixed() +
    theme_void()
  return(p)
}
...
plotLetter <- function(letterList, whichLetter, dims, showPaths = TRUE, showCentroid = TRUE, showSlope = TRUE, showNodes = TRUE) # , showTightness = TRUE, showLoopDims = TRUE)
{
  ...
  if (showNodes) {
    ...
    p <- plotImageThinned(img, which(img == 1))
  }
  ...
  centroidDat <- data.frame(
    X = centroid_x,
    Y = ranger - centroid_y + 1
  )
  halfCentroidDat <- data.frame(
    X = c(lCentroid[2], rCentroid[2]),
  ...
  if (showSlope) {
    p <- p + geom_point(data = halfCentroidDat, aes(x = X, y = Y, color = I("red"), shape = I(4))) +
...
AddLetterImages <- function(letterList, dims) {
  ...
  r <- lapply(skeletons, function(x) {
    ((x - 1) %% dims[1]) + 1
  })
  c <- lapply(skeletons, function(x) {
    ((x - 1) %/% dims[1]) + 1
  })
