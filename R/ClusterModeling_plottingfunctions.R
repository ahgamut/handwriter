...
plotBuckets <- function(bucketData, plotbucket = "4_BMA") {
  Ydat_rel_m <- cbind(bucketData[, c(1, 2)], t(apply(X = bucketData[, -c(1, 2)], MARGIN = 1, function(x) {
    x / sum(x)
  }))) %>% gather(key = "letterbucket", value = "frequency", -c(writer, doc))
  ...
  p <- ggplot(Ydat_plot, aes(x = writer, y = frequency, fill = factor(doc))) +
    ...
    scale_fill_brewer(palette = "Spectral", name = "Document") +
    coord_flip() +
    theme_bw() +
    xlab("Relative Frequency") +
    ylab("Writer") +
    ggtitle("Relative Bucket Frequencies", subtitle = paste0(plotbucket))
  plot(p)
}
...
plotWrappedDensity <- function(measData, writer_plot, cluster_plot, numbins, density_select, plot_density = FALSE, add_unwrapped_density = FALSE, plot_title = TRUE) {
  angleset <- measData %>%
    filter(writer == writer_plot, cluster == cluster_plot) %>%
    pull(pc_wrapped) %>%
    circular(type = "angles", units = "radians", modulo = "2pi")
  ...
  linedat <- linedat %>%
    pivot_longer(cols = c(wrappedCauchy, vonMises), names_to = "Density", values_to = "values") %>%
    filter(Density %in% density_select)

  p <- ggplot(as.data.frame(angleset)) +
    theme_bw() +
...
plotUnwrappedHalf <- function(measData, writer_plot, cluster_plot, numbins, include_yaxis = FALSE, include_title = FALSE, scaled_xaxis = FALSE) {
  angleset <- measData %>%
    filter(writer == writer_plot, cluster == cluster_plot) %>%
    mutate(pc_wrapped = pc_wrapped / 2)
...
get_diagonalprops_byit <- function(postprobs, numCores = 2, logdocname = "diagonalprop_byit") {
  ...
  ls <- foreach(i = 1:nrow(postprobs[[1]]), .combine = c) %dopar% {
    ...
    it_grid <- t(sapply(postprobs, function(x) {
      as.numeric(x[i, ])
    }))
    if (sum(is.na(it_grid)) > 0) {
      cat(i, "has an NA.")
    }
