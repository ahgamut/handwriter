...
output$plot_image_name <- renderText({
...
observeEvent(input$plotbinarized, {
  values$plot_type <- "binarized image"
})
observeEvent(input$plotthinned, {
  values$plot_type <- "thinned image"
})
observeEvent(input$plotnodes, {
  values$plot_type <- "nodes"
})
observeEvent(input$plotbreaks, {
  values$plot_type <- "breaks"
})
observeEvent(input$plotline, {
  values$plot_type <- "line"
})
observeEvent(input$plotword, {
  values$plot_type <- "word"
})
observeEvent(input$plotgraph, {
  values$plot_type <- "graph"
})
...
output$plot_output <- renderPlot({
  ...
  if (values$plot_type == "line") {
    values$number <- input$linenum
  }
  if (values$plot_type == "word") {
    values$number <- input$wordnum
  }
  if (values$plot_type == "graph") {
    values$number <- input$graphnum
  }

  if (values$plot_type == "binarized image") {
    plotImage(imgList$image)
  } else if (values$plot_type == "thinned image") {
    plotImageThinned(imgList$image, imgList$thin)
  } else if (values$plot_type == "nodes") {
    plotNodes(imgList$image, imgList$thin, imgList$nodes)
  } else if (values$plot_type == "breaks") {
    plotNodes(imgList$image, imgList$thin, imgList$breaks)
  } else if (values$plot_type == "line") {
    plotLine(imgList$letterList, input$linenum, imgList$dims)
  } else if (values$plot_type == "word") {
    plotWord(imgList$letterList, input$wordnum, imgList$dims)
  } else if (values$plot_type == "graph") {
    plotLetter(imgList$letterList, input$graphnum, imgList$dims)
  } else {
    values$plot_type <- "thinned image"
    plotImageThinned(imgList$image, imgList$thin)
  }
...
output$plot_output_title <- renderText({
  paste0("Showing ", values$plot_type, " ", values$number)
})
