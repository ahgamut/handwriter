...
observeEvent(input$undo_crop, {
  ...
  image_write(values$image, "tmp.png")
  values$current_path <- "tmp.png"
})
...
observeEvent(input$crop, {
  if (is.null(input$preprocess_plot_brush)) {
    output$error <- renderText({
      "Please select an area prior to cropping."
    })
  } else {
    output$error <- renderText({
      ""
    ...
    shinyjs::enable("reset_crop")
    shinyjs::enable("undo_crop")
    image_write(values$image, "tmp.png")
    values$current_path <- "tmp.png"
  }
})
...
observeEvent(input$mask, {
  if (is.null(input$preprocess_plot_brush)) {
    output$error <- renderText({
      "Please select an area prior to masking."
    })
  } else {
    output$error <- renderText({
      ""
    ...
    shinyjs::enable("undo_mask")
    shinyjs::enable("reset_mask")
