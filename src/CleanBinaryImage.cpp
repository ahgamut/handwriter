...
arma::mat cleanBinaryImage(arma::mat img)
{
    ...
    for (int row = 0; row < n - 2; row++)
    {
        ...
        {
            if (checkMask(img, row, col, H1MASK)) { matchH(row, col) = 1; }
        }
    }
    for (int row = 0; row < n - 2; row++)
    {
        ...
        {
            if (checkMask(img, row, col, H2MASK)) { matchH(row, col) = 2; }
        }
    }
    for (int row = 0; row < n - 3; row++)
    {
        for (int col = 0; col < p - 2; col++)
        {
            if (checkMask(img, row, col, H3MASK)) { matchH(row, col) = 3; }
        }
    }
    ...
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < p; j++)
        {
            if (proposedRemove(i, j) == 1) { img(i, j) = 1; }
            if (proposedFill(i, j) == 1) { img(i, j) = 0; }
        }
    }
    ...
    for (int row = 0; row < n - 2; row++)
    {
        for (int col = 0; col < p - 2; col++)
        {
            ...
            {
                if (checkStep6(img, row, col)) { img(row + 1, col + 1) = 1; }
            }
        }
    }
    ...
    for (int row = n - 5; row >= 0; row--)
    {
        for (int col = p - 5; col >= 0; col--)
        {
            if (img(row + 2, col + 2) == 0)
            {
                ...
                {
                    if (checkMask(img, row, col, D1MASK) ||
                        checkMask(img, row, col, D2MASK) ||
                        checkMask(img, row, col, D3MASK) ||
                        checkMask(img, row, col, D4MASK) ||
                        checkMask(img, row, col, D5MASK))
                    {
                        img(row + 2, col + 2) = 1;
                        if (checkMask(img, row + 1, col, D1MASK) ||
                            checkMask(img, row + 1, col, D2MASK) ||
                            checkMask(img, row + 1, col, D3MASK))
                        {
                            img(row + 3, col + 2) = 1;
    ...
    for (int row = 0; row < n - 4; row++)
    {
        for (int col = 0; col < p - 4; col++)
        {
            if (img(row + 2, col + 2) == 0)
            {
                ...
                {
                    if (checkMask(img, row, col, U1MASK) ||
                        checkMask(img, row, col, U2MASK) ||
                        checkMask(img, row, col, U3MASK) ||
                        checkMask(img, row, col, U4MASK) ||
                        checkMask(img, row, col, U5MASK))
                    {
                        img(row + 2, col + 2) = 1;
                        if (checkMask(img, row - 1, col, U1MASK) ||
                            checkMask(img, row - 1, col, U2MASK) ||
                            checkMask(img, row - 1, col, U3MASK))
                        {
                            img(row + 1, col + 2) = 1;
...
bool checkMask(arma::mat &img, int row, int col, arma::umat mask)
{
    ...
    if (row < 0 || col < 0 || row + rowCount >= img.n_rows ||
        col + colCount >= img.n_cols)
        return (false);
    ...
    for (int i = 0; i < rowCount; i++)
    {
        ...
        {
            res = res &&
                  (mask(i, j) == img(row + i, col + j) || mask(i, j) == 3);
        }
    }
    return (res);
}
...
bool checkStep6(const arma::mat &img, int row, int col)
{
    ...
    arma::vec ring({img(row, col), img(row, col + 1), img(row, col + 2),
                    img(row + 1, col + 2), img(row + 2, col + 2),
                    img(row + 2, col + 1), img(row + 2, col),
                    img(row + 1, col), img(row, col)});
    for (int i = 0; i < 8; i++)
    {
        if (ring(i) == 1 && ring(i + 1) == 0) connectivityCounter++;
        if (ring(i) == 0) blackNeighborCounter++;
    }

    if ((connectivityCounter == 0 || connectivityCounter == 1) &&
        (blackNeighborCounter >= 0 && blackNeighborCounter <= 2))
        return (true);
    else
        return (false);
}
