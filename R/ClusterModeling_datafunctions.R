modelquantities_wrappedcauchy_share <- function(bucketData, measData, a, b, c, d, e) {
  dat <- list(
    Y = bucketData[, -c(1, 2)], # multinomial data
...
modelquantities_wrappedcauchy_gammaloops <- function(bucketData, measData, clustersinmodel_names) {
  loopratiodata <- measData %>%
    mutate(
      loop1_ratio = loop1_longdist / loop1_shortdist,
  ...
  measData <- measData %>%
    filter(cluster %in% clustersinmodel_names) %>%
    mutate(cluster = factor(cluster, levels = levels(clustersinmodel_names)))
...
qdocs_normslope_mean <- function(bucketData, measData, writers = 1:27, writerIndices = TRUE) {
  meanslope_dat <- measData %>%
    group_by(writer, doc, cluster) %>%
...
qdocs_wrappedcauchy <- function(bucketData, measData, writers, writerIndices = TRUE, clustersinmodel_names = NULL) {
  # if NULL all clusters will be included

  if (!is.null(clustersinmodel_names)) {
    measData <- measData %>%
      filter(cluster %in% clustersinmodel_names) %>%
      mutate(cluster = factor(cluster, levels = levels(clustersinmodel_names)))
  }
...
qdocs_wrappedcauchy_gammaloops <- function(bucketData, measData, writers, writerIndices = TRUE, clustersinmodel_names = NULL) {
  # if NULL all clusters will be included
...
angle <- function(N, M) {
  # gives a value in (-pi,pi), where negative values come from unit vectors below the x axis (kinda weird/not traditional)
...
proclistToData_csafe <- function(proclist, testdoc, traindocs, backuptest, oldtrain) {
  docnames <- sapply(proclist, function(x) {
    substr(x$docname, start = 7, stop = 18) %in% c(testdoc, traindocs, backuptest)
  })
  proclist <- proclist[docnames]

  writers <- as.numeric(unlist(lapply(proclist, function(x) {
    substr(x$docname, start = 2, stop = 5)
  })))
  doc <- unlist(lapply(proclist, function(x) {
    substr(x$docname, start = 7, stop = 18)
  }))
  session <- as.numeric(unlist(lapply(proclist, function(x) {
    substr(x$docname, start = 8, stop = 9)
  })))
  prompt <- unlist(lapply(proclist, function(x) {
    substr(x$docname, start = 12, stop = 14)
  }))
  rep <- as.numeric(unlist(lapply(proclist, function(x) {
    substr(x$docname, start = 17, stop = 18)
  })))
  ...
  for (j in 1:length(proclist)) {
    ...
    letter_measurements$letterbucket <- unlist(lapply(proclist[[j]]$process$letterList, function(x) {
      paste0(length(x$nodes), "_", x$letterCode)
    }))
    letter_measurements$cluster <- sapply(proclist[[j]]$process$letterList, function(x) {
      x$cluster
    })
    letter_measurements$slope <- sapply(proclist[[j]]$process$letterList, function(x) {
      x$characterFeatures$slope
    })
    letter_measurements$pc_rotation <- sapply(proclist[[j]]$process$letterList, function(x) {
  ...
  if (length(testdoc) == 1) {
    ...
    for (i in 1:length(unique(writers))) {
      ...
      if (nrow(temp) == 0) {
        temp <- data %>% filter(writer == unique(writer)[i], doc == backuptest)
      }
      if (i == 1) {
        Ytest <- temp
      } else {
        Ytest <- rbind(Ytest, temp)
      }
    }
  }
  ...
  # Train
  M1 <- Y %>%
    select(writer, doc, cluster, slope, pc_rotation, pc_wrapped) %>%
    mutate(writer = factor(writer, levels = unique(writer)), cluster = factor(cluster, levels = names(Ybuckets[, -c(1, 2)])))
  ...
  train <- list(bucketData = Ybuckets, measData = M)
...
proclistToData_csafe_testonly <- function(proclist, oldtrain, testdoc, backuptest) {
  docnames <- sapply(proclist, function(x) {
    substr(x$docname, start = 7, stop = 18) %in% c(testdoc, backuptest)
  })
  proclist <- proclist[docnames]
  ...
  M1test <- Ytest %>%
    select(writer, doc, cluster, slope, pc_rotation, pc_wrapped) %>%
    mutate(writer = factor(writer, levels = unique(writer)), cluster = factor(cluster, levels = names(oldtrain$bucketData[, -c(1, 2)])))
  Mtest <- M1test[with(M1test, order(writer, doc, cluster)), ]
...
proclistToData_cvl_testonly <- function(proclist, testdoc, backuptest, oldtrain) {
  docnames <- sapply(proclist, function(x) {
    substr(x$docname, start = 6, stop = 6) %in% c(testdoc, traindocs, backuptest)
  })
  proclist <- proclist[docnames]

  writers <- as.numeric(sapply(proclist, function(x) {
    substr(x$docname, start = 0, stop = 4)
  }))
  doc <- sapply(proclist, function(x) {
    substr(x$docname, start = 6, stop = 6)
  })
...
proclistToData_iam <- function(proclist, numtest, numtrain, oldtrain, seed) {
  ...
  Y1 <- Ygrouped %>%
    group_by(writer, docgroups, cluster) %>%
    summarise(n = n()) %>%
    tidyr::spread(key = cluster, value = n, fill = 0)
  ...
  # Train
  M1 <- Y %>%
    select(writer, form_id, sent_w_form, cluster, pc_rotation, pc_wrapped) %>%
    mutate(writer = factor(writer, levels = unique(writer)), cluster = factor(cluster, levels = names(oldtrain$bucketData)[-c(1, 2)]))
...
proclistToData_iam_norandom <- function(proclist, numtest, numtrain = 16, oldtrain) {
  ...
  Y <- data %>%
    filter(split == "train") %>%
    mutate(docgroups = (sent_order_w_writer + 3) %% 4 + 1)
  Ytest <- data %>% filter(split == "test")
  ...
  M1 <- Y %>%
    select(writer, form_id, sent_w_form, cluster, pc_rotation, pc_wrapped) %>%
    mutate(writer = factor(writer, levels = levels(Ybuckets$writer)), cluster = factor(cluster, levels = names(oldtrain$bucketData)[-c(1, 2)]))
  M <- M1[with(M1, order(writer, cluster)), ]
...
proclistToData_iam_norandom_nogrouping <- function(proclist, numtest, numtrain = 16, oldtrain) {
  ...
  Y1 <- Y %>%
    group_by(writer, sent_w_writer, cluster) %>%
    summarise(n = n()) %>%
    tidyr::spread(key = cluster, value = n, fill = 0)
